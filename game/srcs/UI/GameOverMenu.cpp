...
void GameOverMenu::update(Keyboard& kb, Mouse& mouse, FontManager& fontManager, int screenW, int screenH, std::stack<std::unique_ptr<MenuScreen>>& stack) {
        ...
        std::vector<float> positions = { screenH * 0.6f - offset, screenH * 0.7f - offset };
        int hovered = hitTestMenuRow(mouse, positions, screenW * 0.38f, screenW * 0.7f, screenH * 0.07f);
        if (hovered >= 0) selector = hovered;
...
void GameOverMenu::draw(RenderContext& tools) {
    ...
    tools.textRenderer.drawText("Play again", "CalliCat", tools.screenW * 0.47f, tools.screenH * 0.6f, tools.screenH * 0.05f, tools.fontManager.getFont("CalliCat"), 0.01f);
    tools.textRenderer.drawText("Main Menu", "CalliCat", tools.screenW * 0.47f, tools.screenH * 0.7f, tools.screenH * 0.05f, tools.fontManager.getFont("CalliCat"), 0.01f);
