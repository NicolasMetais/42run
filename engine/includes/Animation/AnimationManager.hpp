...
class AnimationManager {
    private:
        ...
        /** @brief Recomputes all joint matrices for the skin of lm from current node transforms. */
        void computeJointMatrices(LoadedModel& lm);

    public:
        AnimationManager() = default;

        /** @brief Advances the animation by deltaTime and updates all node transforms and joint matrices. */
        void update(LoadedModel& lm, float deltaTime);
        /** @brief Switches to animation at index and resets the playback timer. */
